...
String[][][] ratings =
{
{{"ratings"},{"direction","left"},{"ncols","1"},{"flow","horizontal"}},
{{"rlike"},{"options","Dislike extremely","Dislike moderately","Dislike slightly","Neither like nor dislike","Like slightly","Like moderately", "Like extremely"}}, 
{{"rfrnd"},{"options","Extremely unfriendly","Moderately unfriendly","Slightly unfriendly","Neither friendly nor unfriendly","Slightly friendly","Moderately friendly", "Extremely friendly"}}, 
{{"rtrst"},{"options","Extremely untrustworthy","Moderately untrustworthy","Slightly untrustworthy","Neither trustworthy nor untrustworthy","Slightly trustworthy","Moderately trustworthy", "Extremely trustworthy"}}, 
};
...
String[][][] explicit =
{
{{"explicit"}, {"choices","ratings"},{"nelem","6"},{"musthave","0"},{"group","0"},{"gminmax","6.6"},{"gplace","0"},{"gorder","0"},{"gsequence","random"}, {"prefix",""}},
{{"jameslike"},{"choices","rlike"},{"stem","<img src='/implicit/user/ybalab/daniel/death1/images/james.jpg'/><br/>How much do you like " + target1Name + "?"}},
{{"brianlike"},{"choices","rlike"},{"stem","<img src='/implicit/user/ybalab/daniel/death1/images/brian.jpg'/><br/>How much do you like " + target2Name + "?"}},
{{"jamesfrnd"},{"choices","rfrnd"},{"stem","<img src='/implicit/user/ybalab/daniel/death1/images/james.jpg'/><br/>How friendly do you think that " + target1Name + " is?"}},
{{"brianfrnd"},{"choices","rfrnd"},{"stem","<img src='/implicit/user/ybalab/daniel/death1/images/brian.jpg'/><br/>How friendly do you think that " + target2Name + " is?"}},
{{"jamestrst"},{"choices","rtrst"},{"stem","<img src='/implicit/user/ybalab/daniel/death1/images/james.jpg'/><br/>How trustworthy do you think that " + target1Name + " is?"}},
{{"briantrst"},{"choices","rtrst"},{"stem","<img src='/implicit/user/ybalab/daniel/death1/images/brian.jpg'/><br/>How trustworthy do you think that " + target2Name + " is?"}},
};
  ...
  for (int i=0;i<ssequence.size();i++) {
   ...
   //  if (h.get(hid+".all.prefix")==null) myitem.prefix.add(""); else myitem.prefix.add(h.get(hid+".all.prefix").get(0));
    ...
    String thisprefix="";
     ...
     if (h.get(sid+".prefix")!=null) thisprefix=h.get(sid+".prefix").get(0);else thisprefix=h.get(self+".all.prefix").get(0);
     myitem.prefix.add(thisprefix);
...
//get the taskid to determine which domain is chosen
	...
	HashMap<String,ArrayList<String>> explicitmap = map2d(emptymap, explicit);
	
	item it = new item();
	
    initsequence(explicitmap,ratingsmap,it);
...
<script language="JavaScript" type="text/javascript" src="/implicit/common/en-us/js/task.js"></script>
    ...
    <style type="text/css">
      .big {font-family: Arial;font-size: 20px;}
      .med {font-family: Arial;font-size: 17px;}
      .sml {font-family: Arial;font-size: 16px;}
    </style>
    <title>Study</title>
...
// #BDEDFF light blue
 ...
 // var minselect = js_minselect[curitem];
  ...
  var curdisplay='<table width="70%" align=center><tr><td class=med><font color="#AAAAAA"><b>Ratings Survey: Item '+(curitem+1)+' of '+js_nitem+'</b></font>'+'<br /> <span  class=med>'+js_prefix[curitem]+'</span><br/><span class=big style="background-color: #FFF8C6">'+js_stem[curitem]+'</span><br/><br/></td></tr></table> <table width="70%" align=center cellpadding="1">';
  ...
  // the finished button
   ...
   if (maxselect>1) {
          curdisplay+='<tr><td class="med">'+prefixskip;
    ...
    curdisplay+='<tr><td class=sml colspan="'+icols+'"><br><br>Select as many items as you wish, turning them blue.<br/>To remove an item from selection, click it once more.<br>When you are done select the &ldquo;Finished and continue&rdquo; button.<br/>Once it turns yellow, you can confirm by clicking it a second time.<br/><br/></td></tr>';
}
   
  
   if ((curitem<=1) && (maxselect<=1)) curdisplay+='<tr><td class=sml colspan="'+icols+'">Selecting an answer once colors it yellow.<br/> You can change your answer by selecting another option. <br /> To confirm, click the selected (yellow) button a second time.</td></tr>';

curdisplay+='</table>';
/***
curdisplay+='</table><table width="90%" align=right>';

          curdisplay+='<tr><td align="right" class="med"><br/><br/>'+prefixskip;
